...
def cal_overlap(args):
    ...
    with open(args.output_path, mode) as f:
        if mode == "w":
            f.write(
                f"dump_1,dump_2,len_dump_1,len_dump_2,overlap_ratio1,overlap_ratio2,overlap_ratio3\n"
            )

        for i in range(len(dump_paths)):
            dump_1 = re.search(r"\d{4}-\d{2}", dump_paths[i])[0]
            print(f"reading {dump_paths[i]}...", file=sys.stderr, end="", flush=True)
            ...
            for j in range(i + 1, len(dump_paths)):
                ...
                if mode == "a" and (
                    len(
                        df_old[
                            (df_old["dump_1"] == dump_1) & (df_old["dump_2"] == dump_2)
                        ]
                    )
                    != 0
                    or len(
                        df_old[
                            (df_old["dump_1"] == dump_2) & (df_old["dump_2"] == dump_1)
                        ]
                    )
                    != 0
                ):
                ...
                print(
                    f"reading {dump_paths[j]}...", file=sys.stderr, end="", flush=True
                )
                ...
                print(
                    f"done: url overlap {overlap_ratio1:.1F}%",
                    file=sys.stderr,
                    flush=True,
                )
