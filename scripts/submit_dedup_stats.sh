...
CACHE=$3
NUM_PARTITIONS=${4:-50}
NUM_PARTITIONS_PROPAGATION=${5:-$(($NUM_PARTITIONS * 4))}

du -hs "$INPUT" > /dev/null &
du -hs "$CACHE" > /dev/null &
...
echo "$(date -Iseconds) $JOB_ID DedupFilterStatistics ssh abci -L8080:$(hostname):8080" >> /scratch/$USER/spark-ui-monitoring

mkdir -p /scratch/$USER/spark-exlog

"$SPARK_HOME/bin/spark-submit" \
    --class com.worksap.nlp.uzushio.lib.runners.DedupFilterStatistics \
    --master $SPARK_MASTER \
    ...
    --input="$INPUT" \
    --stats="$CACHE" \
    --output="$OUTPUT" --partitions=$NUM_PARTITIONS \
    --filter=large-freq-paragraphs
